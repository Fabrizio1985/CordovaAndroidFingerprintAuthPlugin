...
public class FingerprintUiHelper extends FingerprintManager.AuthenticationCallback {
    ...
    private final FingerprintAuth mFingerprintAuth;
    ...
    public static class FingerprintUiHelperBuilder {
        ...
        public FingerprintUiHelperBuilder(Context context, FingerprintManager fingerprintManager, FingerprintAuth fingerprintAuth) {
     ...
     */
    public FingerprintUiHelper(Context context, FingerprintManager fingerprintManager,
            ImageView icon, TextView errorTextView, Callback callback, FingerprintAuth fingerprintAuth) {
        mFingerprintManager = fingerprintManager;
        mIcon = icon;
    ...
    public void startListening(FingerprintManager.CryptoObject cryptoObject) {
        ...
        int ic_fp_40px_id = mContext.getResources()
                .getIdentifier("ic_fp_40px", "drawable", mFingerprintAuth.packageName);
    ...
    public void onAuthenticationFailed() {
        ...
        if (mAttempts > mFingerprintAuth.mMaxAttempts) {
    ...
    public void onAuthenticationSucceeded(FingerprintManager.AuthenticationResult result) {
        ...
        int ic_fingerprint_success_id = mContext.getResources()
                .getIdentifier("ic_fingerprint_success", "drawable", mFingerprintAuth.packageName);
        mIcon.setImageResource(ic_fingerprint_success_id);
        int success_color_id = mContext.getResources()
                .getIdentifier("success_color", "color", mFingerprintAuth.packageName);
        mErrorTextView.setTextColor(
                mErrorTextView.getResources().getColor(success_color_id, null));
        int fingerprint_success_id = mContext.getResources()
                .getIdentifier("fingerprint_success", "string", mFingerprintAuth.packageName);
    ...
    private void showError(CharSequence error) {
        int ic_fingerprint_error_id = mContext.getResources()
                .getIdentifier("ic_fingerprint_error", "drawable", mFingerprintAuth.packageName);
        mIcon.setImageResource(ic_fingerprint_error_id);
        mErrorTextView.setText(error);
        int warning_color_id = mContext.getResources()
                .getIdentifier("warning_color", "color", mFingerprintAuth.packageName);
        mErrorTextView.setTextColor(
    ...
    Runnable mResetErrorTextRunnable = new Runnable() {
        ...
        public void run() {
            int hint_color_id = mContext.getResources()
                    .getIdentifier("hint_color", "color", mFingerprintAuth.packageName);
            mErrorTextView.setTextColor(
                    mErrorTextView.getResources().getColor(hint_color_id, null));
            int fingerprint_hint_id = mContext.getResources()
                    .getIdentifier("fingerprint_hint", "string", mFingerprintAuth.packageName);
            mErrorTextView.setText(
